...
public class selectQueryHandler {
    ...
    private final String SELECT_ALL_QUERY = "^select \\* from (?<tablename>\\w+)(?<whereclause>\\p{javaWhitespace}+where\\p{javaWhitespace}+\\w+((\\p{javaWhitespace}*(=|<=|<|>|>=|<>)\\p{javaWhitespace}*((-?\\d+(\\.\\d+)?)|(\"([[\\p{Graph}&&[^\"\']]\\p{javaWhitespace}*])+\")|(\\d{4}-\\d{2}-\\d{2}_\\d{2}:\\d{2}:\\d{2})|(\\d{4}-\\d{2}-\\d{2})))|(\\p{javaWhitespace}+is null)|(\\p{javaWhitespace}+is not null)))?$";
